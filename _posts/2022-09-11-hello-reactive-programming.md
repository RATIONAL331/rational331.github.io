...
* 리액티브 초기 단계에서는 대부분 라이브러리의 데이터 흐름은 Observable에서 Observer에게 푸시되는 방식이었습니다.
* 푸시 모델은 요청하는 횟수를 최소화하여 전체 시간이 최적화되기 때문에, RxJava 1.x를 비롯한 유사 라이브러리들은 데이터 푸시를 위해 설계되었습니다.
